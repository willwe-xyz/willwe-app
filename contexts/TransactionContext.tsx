...
interface TransactionContextValue {
  executeTransaction: <T extends ethers.ContractTransactionResponse>(
    chainId: string,
    transactionFn: () => Promise<T>,
    options?: TransactionOptions
  ) => Promise<{ tx: T; receipt: TransactionReceipt; hash: string } | null>;
  isTransacting: boolean;
  currentHash: string | null;
  error: Error | null;
}
...
// Create context with typed default value
...
export const TransactionProvider: FunctionComponent<TransactionProviderProps> = ({
  ...
  const executeTransaction = useCallback(async <T extends ethers.ContractTransactionResponse>(
    chainId: string,
    transactionFn: () => Promise<T>,
    options: TransactionOptions = {}
  ): Promise<{ tx: T; receipt: TransactionReceipt; hash: string } | null> => {
    ...
    try {
      ...
      if (receipt.status === 1) {
        ...
        return { tx, receipt: formattedReceipt, hash };
      } else {
